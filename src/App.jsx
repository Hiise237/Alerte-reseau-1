...
export default function AlerteReseau() {
  ...
  const STORAGE_KEY = "alerte-reseau:reports";
  ...
  const handleAdvance = async (id) => {
    const next = reports.map((r) => {
      if (r.id !== id) return r;
      const idx = STATUTS.indexOf(r.status);
      return { ...r, status: STATUTS[(idx + 1) % STATUTS.length] };
    });
    setReports(next);
    await saveReports(next);
  };
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto px-4 py-8 sm:py-12">
        ...
        >
          ...
          {storageError && (
            <div style={{ color: PALETTE.urgent, fontSize: "12px" }} className="mt-2">
              Le signalement est affiché mais n'a pas pu être sauvegardé durablement.
            </div>
          )}
        </form>
        ...
        >
          MVP — les signalements sont enregistrés dans ce navigateur. Pour un
          partage entre plusieurs personnes, une base de données (ex.
          Supabase) doit être ajoutée.
        </div>
      </div>
    </div>
  );
